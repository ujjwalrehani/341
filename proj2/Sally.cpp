   ...
   istrm(input_stream)  // use member initializer to bind reference
{
   ...
   symtab[">="]   =  SymTabEntry(KEYWORD,0,&doGreatEqual) ;
   ...
   symtab["!"]   =  SymTabEntry(KEYWORD,0,&doExcl) ;
...
void Sally::doLess(Sally *Sptr){
   ...
   if ( Sptr->params.size() < 2 ) {
      throw out_of_range("Need two parameters for comparison.") ;
   }
   p1 = Sptr->params.top() ;
   Sptr->params.pop() ;
   p2 = Sptr->params.top() ;
...
void Sally::doLessEqual(Sally *Sptr){
 ...
 void Sally::doGreatEqual(Sally *Sptr){
	Token p1, p2 ;
...
void Sally::doDup(Sally *Sptr){
   ...
   if ( Sptr->params.size() < 1 ) {
      throw out_of_range("Need one parameter for duplication.") ;
   }
   p1 = Sptr->params.top() ;
   p2 = p1;
   Sptr->params.pop();
...
void Sally::doDrop(Sally *Sptr){
	Token p1;
	if ( Sptr->params.size() < 1 ) {
      throw out_of_range("Need one parameter for removal.") ;
   }
   p1 = Sptr->params.top();
   Sptr->params.pop();
...
void Sally::doRot(Sally *Sptr){
	...
	if ( Sptr->params.size() < 3 ) {
      throw out_of_range("Need three parameters for rotaion") ;
   }
...
void Sally::doAnd(Sally *Sptr){
	Token p1;
	if ( Sptr->params.size() < 2 ) {
      throw out_of_range("Need two parameters") ;
   }
